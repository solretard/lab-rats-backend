...
/**
 * Checks whether a wallet holds at least one NFT from a given issuer.
 * Used to gate the Dredd Doctor board — no signing involved, just a
 * public read of the wallet's own NFTs.
 */
async function ownsNftFromIssuer(ws, wallet, issuer) {
  let allNfts = [];
  let marker;
  do {
    const resp = await send(ws, 'account_nfts', {
      account: wallet,
      limit: 400,
      ...(marker ? { marker } : {}),
    });
    allNfts = allNfts.concat(resp.result?.account_nfts || []);
    marker = resp.result?.marker;
  } while (marker);

  const owned = allNfts.filter((n) => n.Issuer === issuer);
  return { owns: owned.length > 0, tokenIds: owned.map((n) => n.NFTokenID) };
}
